...
# H-Net "tokens" are single bytes, but clients (EuroEval/LiteLLM) budget
# max_tokens in subword-tokenizer units (~4 bytes/token for Latin text, more
# for Cyrillic/CJK). Requested generation budgets are multiplied by this so
# structured outputs (e.g. NER JSON) don't get truncated mid-answer.
GEN_BYTE_SCALE = 4.0
...
def _common_params(body: dict) -> dict:
    ...
    return {
        # Scale the client's token budget to bytes (clamped later in _generate)
        "max_tokens": int(
...
def main():
    ...
    parser.add_argument("--max-gen-tokens", type=int, default=4096,
                        help="Hard cap on generated BYTES per request (applied "
                             "after --gen-byte-scale).")
    parser.add_argument("--max-context-length", type=int, default=8192)
    parser.add_argument("--gen-byte-scale", type=float, default=4.0,
                        help="Multiply the client's requested max_tokens by this "
                             "to convert subword-token budgets to bytes (H-Net "
                             "generates one byte per step). 1.0 disables scaling.")
    args = parser.parse_args()

    global MODEL, TOKENIZER, DEVICE, MODEL_NAME, MAX_GEN_TOKENS, MAX_CONTEXT_LENGTH, GEN_BYTE_SCALE
    print("Loading H-Net...")
    ...
    GEN_BYTE_SCALE = args.gen_byte_scale
